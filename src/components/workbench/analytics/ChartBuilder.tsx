...
function DisplayOptionsDropdown({
  ...
  const valueDecimals = displayOptions.valueDecimalPlaces ?? 2;
  const percentDecimals = displayOptions.percentDecimalPlaces ?? 1;
    ...
    <div ref={dropdownRef} className="relative inline-flex">
      ...
      {open ? (
        <div className="absolute left-0 top-full z-50 mt-2 w-[380px] rounded-2xl border border-border bg-background p-3 shadow-xl">
          ...
          <div className="mt-0.5 !text-[12px] leading-4 text-muted-foreground">
            Only options relevant to the selected chart type are shown.
          </div>
          ...
          <div className="mt-4 space-y-3">
            <NativeSelectOption<ChartTitlePosition>
              label="Title position"
              value={displayOptions.titlePosition ?? "left"}
              options={[
              ...
              onChange={(value) =>
                updateDisplayOptions({ titlePosition: value })
              }
            />

            <OptionGroup title="Number formatting">
              <div className="grid grid-cols-2 gap-3">
                <NumberOption
                  ...
                  onChange={(value) =>
                    updateDisplayOptions({
                      valueDecimalPlaces: clampNumber(value, 0, 6),
                    })
                  }
                />
                ...
                <NumberOption
                  ...
                  onChange={(value) =>
                    updateDisplayOptions({
                      percentDecimalPlaces: clampNumber(value, 0, 4),
                    })
                  }
                />
              </div>
            ...
            {isPie ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                <NativeSelectOption<ChartLabelMode>
                  ...
                  onChange={(value) =>
                    updateDisplayOptions({ labelMode: value })
                  }
                />
              </div>
            ) : null}
...
export function ChartBuilder() {
  ...
  function resetTitleToAuto() {
    ...
    setConfig((current) => ({
      ...current,
      title: getDynamicChartTitle(current),
    }));
  }
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.35rem] border border-border bg-background !text-[13px] shadow-sm">
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2 bg-muted/[0.18] px-4 py-2">
        ...
        <ToolChip
          label="Shown columns"
          value={formatCount(safeGroupableColumns.length)}
        />
        <ToolChip
          label="Numeric columns"
          value={formatCount(numericColumns.length)}
        />
              ...
              <div className="space-y-1.5">
                ...
                {isTitleCustom ? (
                  <button
                    ...
                    className="!text-[12px] font-semibold text-muted-foreground transition hover:text-foreground"
                  >
